...
# factor가 1일시 xavier_initializer이며 이 값들을 조절함으로서 여러가지 버전의 초기화 방법을 만들 수 있다.
# batch_normalization(x,train_phase) # convolution or fc연산 후에 나오는 값을 넣어주면 된다.
# flatten(input_data) # rank가 2 혹은 4인 데이터를 넣어주면 이를 [~]과 같은 rank가 1인 데이터로 만들어준다.(여기서는 사용안함)
##########################################################
